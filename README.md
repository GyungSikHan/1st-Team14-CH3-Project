...
# ⚡ 30초 요약 (TL;DR)
- **SYMBIO**: Unreal Engine 5.5 기반 4인 팀 FPS 프로젝트로, 컴포넌트 기반 구조를 활용해 무기·전투·데미지 시스템을 C++로 설계 및 구현
- **핵심 구현**
	- i. **Weapon Base + Component 구조 설계**: WeaponComponent / StatusComponent 등으로 기능을 분리하고, ACWeapon 베이스 클래스로 장착·조준·발사·재장전을 통합 관리  
	- ii. **발사 & 데미지 파이프라인 구축**: LineTrace + Projectile + Delegate 기반으로 총알 충돌 → HitData → TakeDamage까지 흐름 일원화  
	- iii. **Aim / Recoil / 카메라 연출 구현**: Timeline + Curve 기반 FOV 보간, 반동·카메라 쉐이크·데칼·파티클 등 FPS 전투 연출 전반 구현  
- **가장 어려웠던 점 → 해결**
  - 몽타주 중첩 시 State가 복구되지 않아 행동이 막히는 문제  
    → `OnMontageEnded` 델리게이트를 연결해 중단된 몽타주도 강제로 상태 복구 처리
- **바로 보기**
